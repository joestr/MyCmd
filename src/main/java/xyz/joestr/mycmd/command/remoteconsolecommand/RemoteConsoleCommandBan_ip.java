package xyz.joestr.mycmd.command.remoteconsolecommand;
...
public class RemoteConsoleCommandBan_ip {
	
	MyCmd plugin = null;
	
	public RemoteConsoleCommandBan_ip(MyCmd myCmd) {
		
		this.plugin = myCmd;
	}
	
	public boolean process(RemoteConsoleCommandSender remoteConsoleCommandSender, String[] args) {
		...
		if(args.length >= 2) {
			...
			if(!args[0].matches(p.toString())) {
				...
				remoteConsoleCommandSender.sendMessage(this.plugin.pluginPrefix + ChatColor.RED + "Es muss eine gültige IP-Adresse angegeben werden.");
				return true;
			}
			...
			Bukkit.getServer().getBanList(Type.IP).addBan(args[0], message, null, "REMOTECONSOLE");
		...
		remoteConsoleCommandSender.sendMessage(this.plugin.pluginPrefix + this.plugin.usageMessage("/ban-ip <IP-Adresse> <Grund ...>"));
		return true;
	}
}
