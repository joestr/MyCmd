package xyz.joestr.mycmd.command.proxiedcommand;
...
public class ProxiedCommandBan_ip {
	
	MyCmd plugin = null;
	
	public ProxiedCommandBan_ip(MyCmd myCmd) {
		
		this.plugin = myCmd;
	}
	
	public boolean process(ProxiedCommandSender proxiedCommandSender, String[] args) {
		...
		if(args.length >= 2) {
			...
			if(!args[0].matches(p.toString())) {
				...
				proxiedCommandSender.sendMessage(this.plugin.pluginPrefix + ChatColor.RED + "Es muss eine gültige IP-Adresse angegeben werden.");
				return true;
			}
			...
			Bukkit.getServer().getBanList(Type.IP).addBan(args[0], message, null, "PROXY");
		...
		proxiedCommandSender.sendMessage(this.plugin.pluginPrefix + this.plugin.usageMessage("/ban-ip <IP-Adresse> <Grund ...>"));
		return true;
	}
}
