...
public class TabCompletePardon_ip implements TabCompleter {
	
	public TabCompletePardon_ip(MyCmd myCmd) {}
		...
		if(sender instanceof Player) {
			...
			if(arg.length <= 1) {
				...
				if(player.hasPermission("mycmd.command.ban")) {
					...
					for(BanEntry be : Bukkit.getServer().getBanList(Type.IP).getBanEntries()) { list.add(be.getTarget()); }
